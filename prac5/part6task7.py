...
def make_model(game, start_state):
    ...
    dot.node(f'red: {start_state[3]}')
    dot.node(f'blue: {start_state[4]}')
    dot.node(f'green: {start_state[5]}')

    dot.render(outfile='tttm.svg')


START_STATE = tttm.State(
    ['alice', 'bob'][randint(2)], *(['east room', 'west room'][randint(2)] for _ in range(5))
)

make_model(tttm.game, START_STATE)
